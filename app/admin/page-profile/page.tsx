...
const REGION_MAP: Record<string, string> = Object.fromEntries(
  REGION_OPTIONS.map((region) => [region.value, region.label]),
);
  ...
  return (
    <div className="flex items-start gap-4 py-4 border-b border-foreground/5 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-textGrey uppercase tracking-widest mb-0.5">
          {label}
        </p>
        <p
          className={`text-base font-medium truncate ${
            muted ? "text-textGrey" : "text-foreground"
          }`}
        >
          {value || "—"}
        </p>
...
function EditDrawer({
  ...
  user: UserProfile;
  onClose: () => void;
  onSave: (data: { name: string; regionCode: string }) => Promise<void>;
}) {
        ...
        <div className="flex items-center justify-between px-8 py-6 border-b border-foreground/10 bg-foreground/2">
          <div>
            ...
            <p className="text-textGrey text-sm mt-1">
              Update your identity and region
            </p>
          </div>
          <button
            onClick={onClose}
      ...
      <div className="mx-auto mb-20 flex max-w-6xl flex-col gap-10 p-6 md:p-10">
        ...
        <div className="flex flex-col gap-8 lg:flex-row">
          ...
          <div className="flex-1 space-y-6">
            ...
            <Skeleton className="h-64 w-full rounded-3xl border border-foreground/10" />
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex flex-col gap-10 p-10 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col gap-4 max-w-md bg-red-50 border border-red-100 p-6 rounded-2xl">
          ...
          <button
            onClick={() => router.push("/admin/page-sos")}
        ...
        <div className="flex flex-wrap gap-6 items-center justify-between bg-surface border border-foreground/5 p-8 rounded-3xl shadow-sm">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-lg text-textGrey font-medium">
              Manage your administrator identity and assigned region.
            </p>
          </div>
        </div>
            ...
            <div className="bg-surface border border-foreground/10 shadow-sm rounded-3xl p-8 flex flex-col items-center gap-6">
              ...
              <div className="text-center space-y-2 w-full">
                ...
                <div className="flex items-center justify-center gap-2 bg-foreground/5 py-1.5 px-4 rounded-full w-fit mx-auto border border-foreground/10">
                  <span
                    className={`w-2.5 h-2.5 rounded-full shrink-0 ${role.dot} shadow-sm`}
                  />
              ...
              <div className="w-full bg-primary/10 border border-primary/20 rounded-2xl px-5 py-4 text-center relative overflow-hidden">
                ...
                <p className="text-xs text-primary/80 font-bold uppercase tracking-widest mb-1">
                  Assigned Region
                </p>
          ...
          <div className="flex-1 space-y-6">
            <div className="bg-surface border-2 border-green-500/20 shadow-sm rounded-3xl px-8 py-4 relative overflow-hidden">
              ...
              <div className="flex justify-between items-center pt-4 pb-4 border-b border-foreground/10">
                <h3 className="text-xs text-green-600 font-bold uppercase tracking-widest">
                  Account Details
                </h3>
                <button
                  onClick={() => setEditOpen(true)}
                  className="text-sm font-bold text-green-600 bg-green-500/10 px-4 py-1.5 rounded-full hover:bg-green-500/20 transition-all"
                >
                  Edit Identity
                </button>
              </div>
            ...
            <div className="bg-surface border-2 border-orange-500/20 shadow-sm rounded-3xl px-8 py-4 relative overflow-hidden">
              <div className="absolute top-0 left-0 w-1.5 h-full bg-orange-500/80" />
              <div className="pt-4 pb-4 border-b border-foreground/10">
                <h3 className="text-xs text-orange-600 font-bold uppercase tracking-widest">
                  Admin Access
                </h3>
              </div>
              <InfoRow
                label="Workspace"
                value="BorNEO AI control panel"
                icon={
                  <svg
                    width="18"
                    height="18"
